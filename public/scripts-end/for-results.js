...
function cblp(data) {
  for (var r = 0; r < data.numberOfNewCells; r++) {
    var newCell = $("#team-" + data.cell[r].teamNumber + " #main-output-table #cell" + data.cell[r].coordY + "-" + data.cell[r].coordX);
  ...
  for (var t = 0; t < data.numberOfNewLogs; t++) {
    var newLog = data.newLog[t];
    var elNewLog = document.createElement("p");
    elNewLog.innerText = newLog;
    $("#log").append(elNewLog);
  }

  for (var p = 0; p < data.numberOfNewPenalties; p++) {
...
var subscribe = function subscribe(url, cb) {
  $.ajax({
    method: 'GET',
    url: url,
